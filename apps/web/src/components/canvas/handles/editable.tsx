...
const EditableHandle = React.forwardRef<HTMLDivElement, EditableHandleProps>(
  (
    ...
    return (
      <div ref={ref} title={label} className={cn("group relative", wrapperClassName)}>
        <BaseHandle position={position} className={handleClassName} id={handleId} {...handleProps} />
        <div
        ...
        >
          ...
          <div className="flex shrink-0 items-center gap-1">
            ...
            >
              <Button variant="ghost" size="icon" className="size-4 [&_svg]:size-3.5">
                <Edit2 />
              </Button>
            </EditableHandleDialog>
            <Button variant="ghost" size="icon" className="size-4 [&_svg]:size-3.5" onClick={() => onDelete(handleId)}>
              <Trash />
            </Button>
          </div>
        </div>
